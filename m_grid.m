...
if  sscanf(a(1:3),'%f') >6.0 & ~ispc,
  patch('xdata',X(:),'ydata',Y(:),'zdata',-LARGVAL*ones(size(X(:))),'facecolor',get(gca,'color'),...
...
patch(reshape(u2x(kk),size(kk,1),size(kk,2))',...
      ...
      repmat(LARGVAL  ,size(kk,2),size(kk,1)),'w','edgecolor','k','clip','off','tag','m_grid_fancybox1');
patch(reshape(l2x(kk),size(kk,1),size(kk,2))',...
      reshape(l2y(kk),size(kk,1),size(kk,2))',...
      repmat(LARGVAL-1,size(kk,2),size(kk,1)),'k','clip','off','tag','m_grid_fancybox1');
...
line(reshape(mean(l2x(:,kk)),k1,k2),reshape(mean(l2y(:,kk))',k1,k2),...
     repmat(LARGVAL,k1,k2),'color','k','clip','off','tag','m_grid_fancybox2');
