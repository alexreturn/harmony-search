...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%set(gca,'fontsize',20);
%figure, plot(x_valueR,n_countR,'Blue');
%set(gca,'fontsize',20);
%hold on;
%   line([HS_bestF, HS_bestF], ylim, 'LineWidth', 2, 'Color', 'r');
%   hold on;
 % line([HS_bestit(1), HS_bestit(1)], ylim, 'LineWidth', 2, 'Color', 'r');
  %hold on;
  % line([HS_bestit(2), HS_bestit(2)], ylim, 'LineWidth', 2, 'Color', 'g');
   % hold on;
  %  line([HS_bestit(3), HS_bestit(3)], ylim, 'LineWidth', 2, 'Color','m');%'k
  %     hold on;
  %   line([HS_bestit(4), HS_bestit(4)], ylim, 'LineWidth', 2, 'Color', 'k');
   %hold on;
   %line([HS_bestit(5), HS_bestit(5)], ylim, 'LineWidth', 2, 'Color', 'y');
  % hold on;
% legend('\color{Blue} Histogram','\color{red} Threshold 1','\color{green} Threshold 2');

%   legend('\color{Blue} Histogram','\color{red} Threshold 1','\color{green} Threshold 2','\color{magenta} Threshold 3');

 %  legend('\color{Blue} Histogram','\color{red} Threshold 1','\color{green} Threshold 2','\color{magenta} Threshold 3','\color{black} Threshold 4','\color{yellow} Threshold 5');
%Prepare results to be show
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
IoutCrop=imcrop(Iout,[142.5 183.5 345 100]);
bw=IoutCrop;
bw(IoutCrop<BwInver)=0;
bwInverse = ~bw;
...
%figure,imshow(Iout) ;
imgDisk = imdilate(bwpure,strel('disk',5)) 
% figure,imshow(imgDisk),title('imgDisk');
%imgDisk=imfill(imgDisk,'holes');
%lo=canny(double(bwpure));
lo=edge(double(imgDisk),'canny');
testro=lo;
%I2=imcrop(testro,[136 133 285 137]);
%I2=imcrop(testro,[147.5 206.5 271 62]);
I2=imcrop(testro);
...
rgb = imoverlay(I, line_sebelumnya, [1 1 0.5]);
imwrite(Iout,'CitraThreshold.jpg','jpg','Comment','My JPEG file')

imwrite(rgb,'CitraEdgeGabung.jpg','jpg','Comment','My JPEG file')

rgb = imoverlay(rgb, lf, [1 0 0.5]);

rgb2 = imoverlay(I, lf, [1 0 0.5]);

imwrite(rgb2,'CitraEdge.jpg','jpg','Comment','My JPEG file')
coast_line=lf;
%line_ref=lineFin;
%
%     
